...
function WebsiteForm({ site, onClose }: { site: WebsiteRecord | null; onClose: () => void }) {
    ...
    return <form className="space-y-4 bg-slate-900 p-6 text-slate-100" onSubmit={event => {
        ...
        <div><label htmlFor="website-url">URL *</label><input id="website-url" type="text" inputMode="url" autoCapitalize="none" autoCorrect="off" spellCheck={false} className={inputClass} required maxLength={255} placeholder="https://example.com" value={form.data.url} onChange={e => form.setData('url', e.target.value)} />{form.errors.url && <p role="alert" className="text-rose-400 text-sm">{form.errors.url}</p>}</div>
